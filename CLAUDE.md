...
- **S3 client**: `providers/s3_client.rs` — shared AWS SDK S3 client factory plus `describe_s3_error` (the only way to turn an `SdkError` into user-facing text — plain `Display` prints just "service error" and drops the code/message) and `is_transient_s3_error` (5xx/429, throttling and timeout codes, dropped connections — what a retry may wait out; `commands/lazy_sync.rs` retries list pages with exponential backoff only on these, and never on 403/404); provider adapters in `providers/aws/`, `providers/minio/`, `providers/rustfs.rs`. The R2 adapter is NOT under `providers/` — it lives at top-level `src-tauri/src/r2/`
