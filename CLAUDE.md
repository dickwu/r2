...
- CI (`.github/workflows/ci.yml`) runs, on a 4-platform matrix (macOS arm64/x64, Windows, Linux): the seven offline acceptance-harness self-tests (`node scripts/r2-audit-manifest.mjs --self-test`, `node scripts/r2-protocol-audit.mjs --self-test`, `node scripts/aws-s3-protocol-audit.mjs --self-test`, `node scripts/provider-fault-audit.mjs --self-test`, `node scripts/vm-boundary-audit.mjs --self-test`, `python3 scripts/vm-powerloss-audit.py --self-test`, `python3 scripts/run-native-nfs-test.py --self-test`), all the gate commands above (Windows only compiles the Rust tests: `cargo test --no-run`), the native NFS steps (Linux: `python3 scripts/run-native-nfs-test.py --sudo`; Windows: `--validate-only`, a prerequisite check that mounts nothing), then `bun run build` and `bun tauri build` — run the gates and the self-tests locally before pushing
- `.github/workflows/windows-native.yml` is the separate Windows behavioral gate (`workflow_dispatch`, or PRs touching `src-tauri/**` or its scripts): a `windows-2022` runner with SDK 10.0.19041.0 runs the full Rust suite plus three real NFS mount/write/read/rename/unmount iterations and uploads `windows-native-nfs-behavior.json`; `docs/engineering/r2-audit/verification.md` says how to land that artifact as acceptance evidence
...
- **S3 client**: `providers/s3_client.rs` — shared AWS SDK S3 client factory plus `describe_s3_error` (the only way to turn an `SdkError` into user-facing text — plain `Display` prints just "service error" and drops the code/message) and `s3_error_class` (`Transient` = 5xx/429, throttling and timeout codes, dropped connections — what a retry may wait out; a mutation gets `OutcomeUnknown` instead). SDK retries are off (`max_attempts(1)`): replayable reads and immutable multipart parts retry only through `providers/operation.rs::execute` (per-endpoint admission, full-jitter backoff, shared deadline, cancellation; `commands/lazy_sync.rs` passes its own list policy and never retries 403/404), while Copy/PUT/Complete/Delete are never blindly re-sent — an uncertain outcome is reconciled first; provider adapters in `providers/aws/`, `providers/minio/`, `providers/rustfs.rs`. The R2 adapter is NOT under `providers/` — it lives at top-level `src-tauri/src/r2/`
