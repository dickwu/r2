...
async fn saved_fingerprints_on(
    ...
    let sql = match provider {
        "aws" => "SELECT access_key_id,secret_access_key,region,endpoint_scheme,endpoint_host,force_path_style FROM aws_accounts WHERE id=?1",
        "minio" => "SELECT access_key_id,secret_access_key,'us-east-1',endpoint_scheme,endpoint_host,force_path_style FROM minio_accounts WHERE id=?1",
        "rustfs" => "SELECT access_key_id,secret_access_key,'us-east-1',endpoint_scheme,endpoint_host,force_path_style FROM rustfs_accounts WHERE id=?1",
        "r2" => "SELECT access_key_id,secret_access_key,'auto',NULL,NULL,1 FROM tokens WHERE account_id=?1",
