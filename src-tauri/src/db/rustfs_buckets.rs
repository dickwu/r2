...
pub async fn list_rustfs_buckets_by_account(account_id: &str) -> DbResult<Vec<RustfsBucket>> {
    ...
    let mut rows = conn
        .query(
            "SELECT id, account_id, name, public_domain_scheme, public_domain_host, is_public, public_path_prefix, created_at, updated_at
             FROM rustfs_buckets WHERE account_id = ?1 ORDER BY name",
...
pub async fn save_rustfs_buckets_for_account(
    account_id: &str,
    buckets: &[(String, Option<String>, Option<String>, bool, Option<String>)],
) -> DbResult<Vec<RustfsBucket>> {
    let conn = get_connection()?.lock().await;
    ...
    for (name, public_domain_scheme, public_domain_host, is_public, public_path_prefix) in buckets {
        conn.execute(
            "INSERT INTO rustfs_buckets (account_id, name, public_domain_scheme, public_domain_host, is_public, public_path_prefix, created_at, updated_at)
