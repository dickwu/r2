...
fn buffer_budget() -> Arc<Semaphore> {
    ...
    BUDGET
        .get_or_init(|| Arc::new(Semaphore::new(BUFFER_BUDGET_MIB as usize)))
        .clone()
}

/// Reserves process-wide bytes before requesting or allocating a payload. Large
/// parts use a replayable file while ordinary parts move their single Vec into ByteStream.
...
pub(crate) async fn fetch_payload(
    ...
    static SPOOL_SLOTS: OnceLock<Arc<Semaphore>> = OnceLock::new();
    let spool_reservation = if spool {
        Some(
            interruptible(
                cancelled,
                paused,
    ...
    let reservation = interruptible(
        cancelled,
        paused,
        buffer_budget().acquire_many_owned(weight),
    )
    .await
    .map_err(ReadError::permanent)?
    ...
    if response.status() == StatusCode::FORBIDDEN {
        ...
        .await
        .map_err(ReadError::permanent)?
        .map_err(|_| ReadError::permanent("needs_auth: source authorization response stalled"))?
        {
    ...
    validate_response(&response, range, total, etag)?;
    ...
    if spool {
        ...
        options.mode(0o600);
        file =
            Some(options.open(&path).await.map_err(|e| {
    ...
    .map_err(|_| ReadError::transient("Source response body stalled"))?
    {
        ...
        if let Some(file) = &mut file {
            interruptible(cancelled, paused, file.write_all(&chunk))
                .await
                .map_err(ReadError::permanent)?
                .map_err(|e| ReadError::permanent(format!("Cannot write relay part cache: {e}")))?;
        } else {
